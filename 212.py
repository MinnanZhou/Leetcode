class Trie:
    ...
    def insert(self, word: str, index) -> None:
        ...
        self.nodes[word[0]].insert(word[1:], index)
    ...
    def findWords(self, board, words):
        ...
        for i in range(1, len(board) - 1):
            for j in range(1, len(board[0]) - 1):
                if board[i][j] in Tree.nodes:
                    self.recur(i, j, Tree, res, board)
        return [words[i] for i in list(set(res))]

    def recur(self, x, y, tree, res, board):
        if tree.isword:
            res.append(tree.isword - 1)
            tree.isword = False
        if board[x][y] in tree.nodes:
            tmp = board[x][y]
            board[x][y] = "#"
            if self.recur(x + 1, y, tree.nodes[tmp], res, board) or \
                    self.recur(x - 1, y, tree.nodes[tmp], res, board) or \
                    self.recur(x, y + 1, tree.nodes[tmp], res, board) or \
                    self.recur(x, y - 1, tree.nodes[tmp], res, board):
                board[x][y] = tmp
                return True
            else:
                board[x][y] = tmp
...
a = Solution()
inp = [["o", "a", "b", "n"], ["o", "t", "a", "e"], ["a", "h", "k", "r"], ["a", "f", "l", "v"]]
inp0 = [["a", "b"], ["d", "c"]]
inp1 = ["abcd"]
inp2 = [["m", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l"],
...
inp3 = ["aaaaaaaaaa", "aaaaaaaaab", "aaaaaaaaac", "aaaaaaaaad", "aaaaaaaaae", "aaaaaaaaaf", "aaaaaaaaag", "aaaaaaaaah",
